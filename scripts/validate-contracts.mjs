...
function parseGlb(buffer) {
  ...
  while (offset < buffer.length) {
    ...
    if (chunkType === 0x4e4f534a) {
      json = JSON.parse(
        buffer.subarray(start, end).toString('utf8').replace(/\0+$/u, '').trimEnd(),
      );
    }
    offset = end;
  }
...
const configuration = validateJson(
  'configuration example',
  files.configurationSchema,
  files.configuration,
);
...
check(
  surfaceMap.geometry.version === manifest.geometry_version,
  'geometry_version diverge entre contratos',
);
check(
  configuration.geometry.id === manifest.geometry_id,
  'configuration geometry_id diverge do manifesto',
);
check(
  configuration.geometry.version === manifest.geometry_version,
  'configuration geometry_version diverge do manifesto',
);
...
check(
  ...
  'GLB canônico deve declarar Draco como obrigatório',
);
console.log('✓ GLB, hash, tamanho, versão e extensão conferidos');

const key = (value) =>
  `${value.mesh_name}\u0000${value.primitive_index}\u0000${value.material_name}`;
const glbBindings = collectBindings(gltf);
const mapBindings = surfaceMap.surfaces.map((surface) => ({
  ...surface.binding,
...
for (const binding of glbBindings)
  check(mapKeys.has(key(binding)), `binding ausente no surface-map: ${JSON.stringify(binding)}`);
for (const binding of mapBindings)
  check(glbKeys.has(key(binding)), `binding inexistente no GLB: ${JSON.stringify(binding)}`);
...
check(
  !surfaceMap.surfaces.some((surface) => /Material\.\d+|[A-Z]{2,}_/u.test(surface.public_name)),
  'identificador técnico vazou para nome público',
);
...
check(
  JSON.stringify(assignmentIds) === JSON.stringify(configurableIds),
  'configuration example não cobre exatamente as superfícies configuráveis',
);
check(
  !assignmentIds.some((surfaceId) => fixedIds.has(surfaceId)),
  'configuration example atribui material a superfície fixa',
);
check(
  Object.values(configuration.assignments).every(
    (materialId) => materialId === material.material_id,
  ),
  'configuration example referencia material diferente do exemplo publicado',
);
...
check(
  glbBindings.length === manifest.statistics.part_count,
  'part_count do manifesto diverge do GLB',
);
check(
  configurableIds.length === manifest.statistics.configurable_surface_count,
  'contagem configurável do manifesto diverge',
);
check(
  fixedIds.size === manifest.statistics.fixed_surface_count,
  'contagem fixa do manifesto diverge',
);
check(
  vertexCount === manifest.statistics.upload_vertex_count,
  'contagem de vértices do manifesto diverge do GLB',
);
check(
  triangleCount === manifest.statistics.triangle_count,
  'contagem de triângulos do manifesto diverge do GLB',
);

check(
  surfaceMap.camera.limits.min_polar_deg <= surfaceMap.camera.default_orbit.polar_deg,
  'órbita polar inicial abaixo do limite',
);
check(
  surfaceMap.camera.default_orbit.polar_deg <= surfaceMap.camera.limits.max_polar_deg,
  'órbita polar inicial acima do limite',
);
check(
  surfaceMap.camera.limits.min_radius_m <= surfaceMap.camera.default_orbit.radius_m,
  'raio inicial abaixo do limite',
);
check(
  surfaceMap.camera.default_orbit.radius_m <= surfaceMap.camera.limits.max_radius_m,
  'raio inicial acima do limite',
);
...
function assertNoPrivateKeys(value, location = '$') {
  if (Array.isArray(value))
    return value.forEach((item, index) => assertNoPrivateKeys(item, `${location}[${index}]`));
  if (!value || typeof value !== 'object') return;
  for (const [field, child] of Object.entries(value)) {
    check(
      !forbiddenPublicKeys.has(field.toLowerCase()),
      `campo privado ${field} encontrado em ${location}`,
    );
