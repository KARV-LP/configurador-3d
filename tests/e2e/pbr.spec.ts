...
async function findConfigurableHit(page: import('@playwright/test').Page) {
...
test('aplica Base Color, Normal, AO e roughness pela experiência oficial', async ({ page }) => {
  ...
  page.on('request', (request) => {
    ...
    if (url.pathname.includes('/public/v1/assets/fabric-kv-002/')) {
      if (/\/(base-color|normal|ao)\.webp$/u.test(url.pathname)) assetRequests.push(url.pathname);
    }
  });
  ...
  const { viewer, hit } = await findConfigurableHit(page);
  if (!hit) return;
  ...
  await expect(page.getByTestId('pbr-status')).toContainText('Escolha onde deseja aplicar');
  ...
  expect(assetRequests.filter((path) => path.endsWith('/base-color.webp'))).toHaveLength(1);
  expect(assetRequests.filter((path) => path.endsWith('/normal.webp'))).toHaveLength(1);
  expect(assetRequests.filter((path) => path.endsWith('/ao.webp'))).toHaveLength(1);
